...
$Host.UI.RawUI.WindowTitle = 'connectsphere-search-service'
...
Write-Host 'Starting search-service on port 8087...'
Write-Host 'Backend directory: C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend'
Write-Host ''
& mvn -pl 'search-service' spring-boot:run "-Dspring-boot.run.workingDirectory=C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend"
if ($LASTEXITCODE -ne 0) {
    Write-Host ''
    Write-Host 'search-service stopped with exit code' $LASTEXITCODE -ForegroundColor Red
}
Write-Host ''
