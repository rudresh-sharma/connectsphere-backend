...
$Host.UI.RawUI.WindowTitle = 'connectsphere-follow-service'
...
Write-Host 'Starting follow-service on port 8082...'
Write-Host 'Backend directory: C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend'
Write-Host ''
& mvn -pl 'follow-service' spring-boot:run "-Dspring-boot.run.workingDirectory=C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend"
if ($LASTEXITCODE -ne 0) {
    Write-Host ''
    Write-Host 'follow-service stopped with exit code' $LASTEXITCODE -ForegroundColor Red
}
Write-Host ''
