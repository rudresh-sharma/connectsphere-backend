...
$Host.UI.RawUI.WindowTitle = 'connectsphere-comment-service'
...
Write-Host 'Starting comment-service on port 8083...'
Write-Host 'Backend directory: C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend'
Write-Host ''
& mvn -pl 'comment-service' spring-boot:run "-Dspring-boot.run.workingDirectory=C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend"
if ($LASTEXITCODE -ne 0) {
    Write-Host ''
    Write-Host 'comment-service stopped with exit code' $LASTEXITCODE -ForegroundColor Red
}
Write-Host ''
