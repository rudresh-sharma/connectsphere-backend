...
$Host.UI.RawUI.WindowTitle = 'connectsphere-notification-service'
...
Write-Host 'Starting notification-service on port 8085...'
Write-Host 'Backend directory: C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend'
Write-Host ''
& mvn -pl 'notification-service' spring-boot:run "-Dspring-boot.run.workingDirectory=C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend"
if ($LASTEXITCODE -ne 0) {
    Write-Host ''
    Write-Host 'notification-service stopped with exit code' $LASTEXITCODE -ForegroundColor Red
}
Write-Host ''
