...
$Host.UI.RawUI.WindowTitle = 'connectsphere-eureka-server'
...
Write-Host ''
& mvn -pl 'eureka-server' spring-boot:run "-Dspring-boot.run.workingDirectory=C:\Users\rudre\Desktop\SocialMedia\connectsphere-backend"
if ($LASTEXITCODE -ne 0) {
    Write-Host ''
    Write-Host 'eureka-server stopped with exit code' $LASTEXITCODE -ForegroundColor Red
}
Write-Host ''
